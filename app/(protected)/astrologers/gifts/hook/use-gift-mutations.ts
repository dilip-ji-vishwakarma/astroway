...
export const UseGiftMutations = (onOpenChange: any) => {
  const {
    handleSubmit,
    control,
    setValue,
  ...
  const onSubmit = async (formProp: any) => {
    try {
      ...
      const response = await apiServices(gifts, "post", formData);
  ...
  return { setValue, isSubmitting, control, handleSubmit, onSubmit, errors };
};
